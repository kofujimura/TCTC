...
This example above uses `_grantRoleByToken`, an `internal` function that is useful when programmatically assigning roles (such as during construction). However, granting the 'minter' or 'burner' role to the actual user account is independent of this contract generation. For example, for a user to obtain mint privilege, they must obtain the requested control token from the specified control token issuer. The following is the case where mint privilege is issued to the issuer of MyToken as a token called Minter Cert.
