...
For each role, tokenController has the mapping of contract IDs, which will hold the list of contracts the participant must own with that role. When _grantRoleToken() is called multiple times, it requires to have the token specified by at least one of the contract IDs. 
